...
object MongoUtils {
  ...
  implicit class MongoDocumentOps(value: Document) {
    def getOptString(key: String): Option[String] = value.get[BsonString](key).map(_.getValue)
    ...
    def getOptDate(key: String): Option[DateTime] =
      value.get[BsonDateTime](key).map(_.getValue).map(millis => new DateTime(millis))
    def getDate(key: String): DateTime        = getValue(getOptDate(key), key)
    def getOptId(key: String): Option[String] = value.get[BsonObjectId](key).map(_.asString().getValue)
    def getId(key: String): String            = getValue(getOptId(key), key)
    def getOptValues[T](key: String)(implicit converter: MongoConverter[T]): Option[Seq[T]] =
      value.get[BsonArray](key).map(_.getValues.asScala.toList.map(doc => converter.toValue(doc.asDocument())))
