...
trait Histogram {
  ...
  def recordFuture[T](codeToBenchmark: => Future[T])(implicit ec: ExecutionContext): Future[T] = {
    ...
    result.foreach(_ => record(System.nanoTime() - before))

    result
  }

  def recordFutureOnEitherRight[P[_]: Monad, T, U](codeToBenchmark: => P[Either[T, U]]): P[Either[T, Unit]] = {
    val before = System.nanoTime()
    val result = codeToBenchmark

    result.map(eitherResult => eitherResult.map(_ => record(System.nanoTime() - before)))
  }
}
