...
object LeagueDayId {
  def unsafe(value: String): LeagueDayId = ValidationErrorException.getOrThrow(fromString(value))

  def fromString(value: String): Validation[LeagueDayId] =
    Validated.condNel(Id.isValid(value), LeagueDayId(UUID.fromString(value)), InvalidLeagueDayId(value))
}
